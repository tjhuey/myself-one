...
String.prototype.isMobileNum = function() {
	return (new RegExp(/^((13[0-9])|(14[4,7])|(15[^4,\D])|(17[0,6,7,8])|(18[0-9]))(\d{8})$/)
			.test(this));
}
 ...
 * @param hiddenId
 */
function uploadImage4Hidden(fileElementId, imageElementId, hiddenId) {
	var path = $("#" + fileElementId).val();
...
//手机验证码调用时间定时器(当点击按钮是input标签时使用)H
function teleCodeTimerH(id){
    ...
    if(timeH == 0){
       ...
       $("#"+id).attr("disabled", false);
    	...
    	myTimeout = setTimeout('teleCodeTimerH("'+ id +'")',1000);
    }
}
...
function timer2(){
    ...
    if(time2 == 0){
       ...
       $("#"+blug2).attr("disabled", false);
    	...
    	objTime = setTimeout('timer2()',1000);
    }
}
...
function showHighLightFront(index){
	$("#child" + index).css({'background-color':'#fff','color':'#fb6e02'});
	$(".pppa a").css("color","#666");
	$(".user-nav-main").css({'background':'url(/public/images/user_sprite.png) no-repeat -32px -135px'});
...
function showHighLightFront2(maxIndex, nowIndex){
	...
	//alert(maxIndex+""+nowIndex);
	if(null != tg){
		if(tg.is(":visible")){
			$("#xf_mem_nav_left div").hide();
			tg.show();
		}else{
			tg.hide();
		}
