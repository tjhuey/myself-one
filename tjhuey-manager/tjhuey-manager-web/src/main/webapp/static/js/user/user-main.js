//添加VIP会员员
$('#addVipUserBtn').on("click", function() {

	var url = getRootPath() + '/pages/user/VipUserAdd.jsp';
	layer.open({
		type : 2,
		title : false,
		area : [ '820px', '450px' ],
...
//添加普通会员
$('#addCommonUserBtn').on("click", function() {
	
	var url = getRootPath() + '/pages/user/CommonUserAdd.jsp';
	layer.open({
		type : 2,
		title : false,
		area : [ '820px', '450px' ],
...
function editUser(userId) {
	var url = getRootPath() + '/user/queryUserDetail?id=' + userId;
...
$('#deleteMulti').on("click", function(){
	...
	var checkbox =  document.getElementsByName("check-input"); 
	var count = 0;
	var url = getRootPath() + '/user/deleteMultiAjax';
	...
	if(flag == true){
		layer.msg('请选择会员！',{time:1500});
	}else{
		layer.confirm('确认删除选中的'+count+'个会员吗？', {
			btn: ['删除','取消'],
			title: '确认删除',
			shadeClose: true 
		}, function(){
...
// 删除会员
function deleteUser(userId, nickName, ele) {
	
	var url = getRootPath() + '/user/deleteAjax';

	layer.confirm('确认删除会员"' + nickName + '"？', {
		btn : [ '删除', '取消' ],
		title : '确认删除',
		shadeClose : true
	}, function() {
