...
(function($) {
  ...
  $.panelslider = function(element, menuicon, options) {
    ...
    if(active.is(':visible') && active[0] != element[0]) {
      $.panelslider.close(function() {
    	...
    	menuicon[0].className = 'fa fa-remove';//dhz
      _slideIn(element, options);
    }
  };
  ...
  $.panelslider.close = function(callback) {
    ...
    var menuicon = $(active[0].getAttribute('menuicon')); //dhz
	menuicon[0].className="fa fa-navicon"; //dhz
  ...
  $(document).bind('click keyup', function(e) {
	...
	var panel = $('.panel');//dhz
    var menuicon = $(panel[0].getAttribute('menuicon')); //dhz
    ...
    if(e.type == 'keyup' && e.keyCode == 27) {
    	...
    	if(!active.is(':visible')){
    		...
    		$.panelslider(panel, menuicon, defaults);
    	}else{
    		menuicon[0].className = 'fa fa-navicon';
  	      	$.panelslider.close();
    	}
	  //dhz-end
  ...
  $.fn.panelslider = function(options) {
    this.click(function(e) {
      var active = $('.ps-active-panel'),
          ...
          menuicon = $(this.getAttribute('menuicon'));  //dhz
      ...
      if (active.is(':visible') && panel[0] == active[0]) {
        ...
        $.panelslider(panel, menuicon, options);//dhz
      }
